...
Mesh* Lab6::CreateMesh(const char *name, const std::vector<VertexFormat> &vertices, const std::vector<unsigned int> &indices)
{
    ...
    //RECAP:VAO,VBO,IBO, VAO: coordonate sii alte atibute, textura etc => de la cpu la gpu
    //nu se pot trimite insttant => trimit mai intai 1 atribut pt fiecare vertex, samd 

    // Set vertex position attribute -> 1 ATRIBUT PT TOTI VERTECSII: v_position in shader
    glEnableVertexAttribArray(0);
    // 3 flotaturi nenormalizate,  pas citire , 0 = offset initial      //DEPLASAMENT
    glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), 0);

    // Set vertex normal attribute -> 2 atribut toti vertecsii!!samd
    //v_normal in shader!
    glEnableVertexAttribArray(1);                   //peste ce zona de memorie trb sa sar pt a ajunge la atribut
    glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(VertexFormat), (void*)(sizeof(glm::vec3)));

    // Set texture coordinate attribute
    //v_texture in shader!
    glEnableVertexAttribArray(2);   
                                       //peste cati bytes sar pt urmat atribut
...
//Trimitere date generale la shader:
void Lab6::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix)
{
    ...
    //Pas1: locatia variabilei in programul de shader cu glGetUniformLocation(int shader_program, "uniform_variable_name_in_shader");
    GLint modelLocation = glGetUniformLocation(shader->GetProgramID(), "Model");

    // TODO(student): Set shader uniform "Model" to modelMatrix
                    //AM LOCATIA LA SHADERE: trimit la acest pointer info cu ajut glUniform
    ...
    //////////////////BONUS: in afara de atributlee vertexului sa fac shader la timpul app
    //1) locatie shader 2) glUnifrom trimit info la  locatie, info este fct din lab de luat timp

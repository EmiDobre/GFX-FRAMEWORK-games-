...
void Lab5::OnInputUpdate(float deltaTime, int mods)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        ...
        //modificar field of view din perspectiva
        //modificare width si height la ortho:
        if (window->KeyHold(GLFW_KEY_Z)) {
            fov += deltaTime * 10 * cameraSpeed;
            left += deltaTime * 10 * cameraSpeed;
    ...
    //schimabre field of view: cat de wide vede:
...
void Lab5::OnMouseMove(int mouseX, int mouseY, int deltaX, int deltaY)
{
    ...
    if (window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT))
    {
        float sensivityOX = 0.001f; //scalam cu "cat de repede misc camera"
        ...
        if (window->GetSpecialKeyState() == 0) {
            ...
            //rotesc dupa axa orizontala Ox: cand mut mouse in sus/jos => deltaY
            //  camera vreau sa se duca in jos sau sus (opus de cum mut mouse)
            //ca sa dau impresia de movement: - !!
            camera->RotateFirstPerson_OX(sensivityOX * -deltaY);

            //Ma mut si tanga dreapta in plan - rotesc dupa Ox care ramane ct

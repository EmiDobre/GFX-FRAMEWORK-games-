...
{
    change = 0;
    cullFace = GL_BACK;  //nu va afisa spatele
    polygonMode = GL_FILL; //la apasare space afiseaza diferit
    ...
    // Create a mesh box using custom data
    {
        ...
        {
            //VertexFormat(pozitie, culoare, normala?)
            //lab: VertexFormat(glm::vec3(-1, -1,  1), glm::vec3(0, 1, 1), glm::vec3(0.2, 0.8, 0.6)),
            // TODO(student): Complete the vertices data for the cube mesh

            //conteaza ordinea!!vertez 0, 1, 2, 3, samd:
        ...
        vector<unsigned int> indices =
        {
           ...
           // 
           //ORDINEA E IMPORTANTA!!!!def fetele din fata si din spate
           // triunghiul 0 1 2 vr sa s evada dar 0 1 4 nu din fata => definim in ord trigonometrica: SE VEDE
           // fata din spate 0 1 4 o def anti trigonometric => nu se vede
        ...
        //AICI!! se creeaza un mesh al cubului cu fct din frame iNITfROMdATA
        //dupa se randeaza deci pun in update
    ...
    // by definition!
    {
        ...
        meshes["teta"] = new Mesh("tetaTeta");
        meshes["teta"]->InitFromData(vertices, indices);
    ...
    // opposing vertex orientations.
    {
        ...
        meshes["patrat"] = new Mesh("patratPatrat");
    ...
    ///BONUS:
    {
        ...
        {
            //VertexFormat(pozitie, culoare, normala?)
             VertexFormat(glm::vec3(0, 0, 1), glm::vec3(0.859, 0.839, 0.184)),
             VertexFormat(glm::vec3(0, 0, 0), glm::vec3(0.184, 1, 0)),
             VertexFormat(glm::vec3(1, 0, 0), glm::vec3(0.859, 1, 1)),
             VertexFormat(glm::vec3(0, 1, 0), glm::vec3(0.678, 0.678, 1)),
        };
        ...
        meshes["teta2"] = new Mesh("tetaTeta2");
        meshes["teta2"]->InitFromData(vertices, indices);
...
//meshurile se creeaza o sg data in init!! 




//se face alt cub cu fct noastar de creare acum, A si B au zone dif de memorie dar aarata la fel!
...
void Lab2::Update(float deltaTimeSeconds)
{
    ...
    RenderMesh(meshes["cube_B"], shaders["VertexNormal"], glm::vec3(-4.5, 0.5f, -4.5f), glm::vec3(0.75f));

    // TODO(student): Draw the tetrahedron
    RenderMesh(meshes["teta"], shaders["VertexNormal"], glm::vec3(0, 0.5f, -1.5f), glm::vec3(0.75f));

    // TODO(student): Draw the square
    RenderMesh(meshes["patrat"], shaders["VertexNormal"], glm::vec3(2.5f, 0.5f, 4.5f), glm::vec3(0.75f));

    //bonus fa un tetraedu unde sa nu se vada gradientul 
    RenderMesh(meshes["teta2"], shaders["VertexNormal"], glm::vec3(1.5f, 0.5f, 1.9f), glm::vec3(0.75f));

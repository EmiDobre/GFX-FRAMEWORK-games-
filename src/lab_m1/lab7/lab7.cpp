...
void Lab7::Init()
{
    ...
    // Light & material properties
    {
        ...
        //bonus:
        lightColor = glm::vec3(0.878f, 0.627f, 0.314f);
    }
}
...
void Lab7::Update(float deltaTimeSeconds)
{
...

    {
        ...
        RenderSimpleMesh(meshes["box"], shaders["LabShader"], modelMatrix, glm::vec3(0.f, 0.5f, 0.f));
    }
    ...
    // Render ground
    {
        ...
        modelMatrix = glm::scale(modelMatrix, glm::vec3(0.25f));    //ultimul atribut e culaorea!
...
void Lab7::RenderSimpleMesh(Mesh *mesh, Shader *shader, const glm::mat4 & modelMatrix, const glm::vec3 &color)
{
    ...
    //trimit info: cate triplete light pos trimit: 1, transmit cu glm value pointer ca se modif poz la moment de timp
    ...
    //2) kd:
    GLint locMaterialKd = glGetUniformLocation(shader->program, "material_kd");  // difuza
    glUniform1f(locMaterialKd, materialKd);
    //3) ks:
    GLint locMaterialKs = glGetUniformLocation(shader->program, "material_ks");  // speculara
    ...
    //BONUS: 5)culaore lumina:
    GLint light_colour = glGetUniformLocation(shader->program, "light_color");
    glUniform3fv(locObject, 1, glm::value_ptr(lightColor));
